...
其中`data`就是临时分配的内存。理论上，这段内存如果只在`Handle`中使用，只在栈上就可以，随着`Handle`退出自动释放。但是由于逃逸分析无法获知是否在`r.Read()`时持有了`data[:]`这个引用，这里会认为`data`有逃逸，所以会将其分配在栈上，并导致后面gc的参与。

回到Rust，最终，`heap`被定义为`Option<Box<[u8]>>`，表示一个可以为`None`，也可以是一个分配在堆上的`[u8]`。`[u8]`这个类型与Go的slice从概念到实现都很类似，是对数组的一种引用。值得注意的是，`[u8]`由于无法在编译时预知引用的长度，所以无法直接在堆上创建一个`[u8]`的变量。
...
由于Rust没有官方的`io.ReadFull()`，所以这里做了自己的实现：
